...
function wxSearchConfirm(e) {
  ...
  console.log("开始搜索" + __that.data.wxSearchData.value)
  if (key == 'back') {
    __goBackFunction();
  } else {
...
function search(inputValue) {
  ...
  if (inputValue && inputValue.length > 0) {
    ...
    wx.request({
      url: 'http://172.16.81.57/thinkphp5/public/index/user/search',
      data: {
        key: inputValue
      },
      method: "POST",
      ...
      success: function (res) {
        ...
        for (var i = 0; i < list.length; i++){
          if (list[i].user_openid == null || list[i].user_openid == ""){//删除未注册的人
            delete list[i]
          }
        }  
        var tipKeys = list;
